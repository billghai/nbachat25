    ...
    if user_teams and any(word in query.lower() for word in ["last", "next", "today", "tonight"]):
        ...
        if "next" in query.lower():
            ...
            if team == "New York Knicks":
                ...
                if series_key in KNOWN_SERIES:
                    ...
                    response = f"New York Knicks play Boston Celtics on 2025-05-07, 7:00 PM PDT (Game 2). Series: {KNOWN_SERIES[series_key]}."
                    return response, False
            ...
            if team == "Minnesota Timberwolves":
                ...
                if series_key in KNOWN_SERIES:
                    ...
                    response = f"The Timberwolves’ next game is Game 1 vs. Warriors on 2025-05-06, 9:30 PM PDT. Series: {KNOWN_SERIES[series_key]}."
                    return response, False
            if team == "Golden State Warriors":
                series_key = "Golden State Warriors vs Minnesota Timberwolves 2025-05-06"
                if series_key in KNOWN_SERIES:
                    logger.debug(f"Using known series for Warriors: {series_key}")
                    response = f"Golden State Warriors play Minnesota Timberwolves on 2025-05-06, 9:30 PM PDT (Game 1). Series: {KNOWN_SERIES[series_key]}."
                    return response, False
            if team == "Indiana Pacers":
                ...
                if series_key in KNOWN_SERIES:
                    ...
                    response = f"Indiana Pacers play Cleveland Cavaliers on 2025-05-06, 7:00 PM PDT (Game 2). Series: Pacers lead 2-0."
                    return response, False
            if team == "Cleveland Cavaliers":
                series_key = "Indiana Pacers vs Cleveland Cavaliers 2025-05-04"
                if series_key in KNOWN_SERIES:
                    logger.debug(f"Using known series for Cavaliers: {series_key}")
                    response = f"Cleveland Cavaliers play Indiana Pacers on 2025-05-06, 7:00 PM PDT (Game 2). Series: Pacers lead 2-0."
                    return response, False
            if team == "LA Clippers":
                series_key = "LA Clippers vs Denver Nuggets 2025-05-03"
                if series_key in KNOWN_SERIES:
        ...
        if "last" in query.lower() and series_keys:
            ...
            if team == "Los Angeles Lakers":
                series_key = "Los Angeles Lakers vs Minnesota Timberwolves 2025-04-30"
                if series_key in KNOWN_SERIES:
                    response = f"The Lakers lost their last game against the Timberwolves on 2025-04-30, with a score of 98-107. Anthony Davis led the team with 30 points and 14 rebounds. Series: {KNOWN_SERIES[series_key]}."
                    return response, False
            if team == "Golden State Warriors":
            ...
            if team == "Minnesota Timberwolves":
                series_key = "Los Angeles Lakers vs Minnesota Timberwolves 2025-04-30"
                if series_key in KNOWN_SERIES:
                    response = f"The Timberwolves won their last game against the Lakers on 2025-04-30, with a score of 107-98. Series: {KNOWN_SERIES[series_key]}."
                    return response, False
            if team == "Indiana Pacers":
                series_key = "Indiana Pacers vs Cleveland Cavaliers 2025-05-04"
                if series_key in KNOWN_SERIES:
        ...
        if team in ["Los Angeles Lakers", "Miami Heat", "LA Clippers", "New York Knicks", "Boston Celtics", "Denver Nuggets", "Golden State Warriors", "Minnesota Timberwolves", "Indiana Pacers", "Cleveland Cavaliers", "Orlando Magic"]:
            if "eliminated" in grok_response.lower() or "no future games" in grok_response.lower():
                ...
                if team == "Minnesota Timberwolves" and "next" in query.lower():
                    response = "The Timberwolves’ next game is Game 1 vs. Warriors on 2025-05-06, 9:30 PM PDT. Series: Series tied 0-0."
                    return response, False
                if team == "Indiana Pacers" and "next" in query.lower():
                    response = "Indiana Pacers play Cleveland Cavaliers on 2025-05-06, 7:00 PM PDT (Game 2). Series: Pacers lead 2-0."
                    return response, False
                if team == "Cleveland Cavaliers" and "next" in query.lower():
                    response = "Cleveland Cavaliers play Indiana Pacers on 2025-05-06, 7:00 PM PDT (Game 2). Series: Pacers lead 2-0."
                    return response, False
